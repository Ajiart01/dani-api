...
__path = process.cwd()
     ...
     async function asmaulhusna(req, res) {            
            ...
            limitAdd(apikey);
            let muslim = JSON.parse(fs.readFileSync(__path +'/lib/muslim/AsmaulHusna.json'))         
      ...
      async function ayatkursi(req, res) {            
            ...
            if (limit) return res.status(429).send({ status: 429, message: 'requests limit exceeded (100 req / day), call owner for an upgrade to premium', result: 'error' })
            limitAdd(apikey);
            let muslim = JSON.parse(fs.readFileSync(__path +'/lib/muslim/ayatkursi.json'))         
            res.status(200).json(muslim)
            }
         ...
         async function bacaansholat(req, res) {           
            ...
            if (limit) return res.status(429).send({ status: 429, message: 'requests limit exceeded (100 req / day), call owner for an upgrade to premium', result: 'error' })
            limitAdd(apikey);
            let muslim = JSON.parse(fs.readFileSync(__path +'/lib/muslim/bacaanshalat.json'))         
            res.status(200).json(muslim)
            }
       ...
       async function niatsholat(req, res) {           
            ...
            if (limit) return res.status(429).send({ status: 429, message: 'requests limit exceeded (100 req / day), call owner for an upgrade to premium', result: 'error' })
            limitAdd(apikey);
            let muslim = JSON.parse(fs.readFileSync(__path +'/lib/muslim/niatshalat.json'))         
            res.status(200).json(muslim)
            }
         ...
         async function tahlil(req, res) {           
            ...
            if (limit) return res.status(429).send({ status: 429, message: 'requests limit exceeded (100 req / day), call owner for an upgrade to premium', result: 'error' })
            limitAdd(apikey);
            let muslim = JSON.parse(fs.readFileSync(__path +'/lib/muslim/tahlil.json'))         
            res.status(200).json(muslim)
            }
         ...
         async function wirid(req, res) {           
            ...
            if (limit) return res.status(429).send({ status: 429, message: 'requests limit exceeded (100 req / day), call owner for an upgrade to premium', result: 'error' })
            limitAdd(apikey);
            let muslim = JSON.parse(fs.readFileSync(__path +'/lib/muslim/wirid.json'))         
            res.status(200).json(muslim)
            }       
         ...
         async function randomquran(req, res) {           
            ...
            if (limit) return res.status(429).send({ status: 429, message: 'requests limit exceeded (100 req / day), call owner for an upgrade to premium', result: 'error' })
            limitAdd(apikey);
            let muslim = JSON.parse(fs.readFileSync(__path +'/lib/muslim/quran.json'))         
       ...
       async function sholat(req, res) {
         try {
            ...
            if (limit) return res.status(429).send({ status: 429, message: 'requests limit exceeded (100 req / day), call owner for an upgrade to premium', result: 'error' })
            limitAdd(apikey);
            let result = await scraper.jadwalsholat(kota)
              res.status(200).json({ status: 200, result: result })
          } catch(err) {
              console.log(err)
     ...
     async function surah(req, res) {
         try {
            ...
            if (limit) return res.status(429).send({ status: 429, message: 'requests limit exceeded (100 req / day), call owner for an upgrade to premium', result: 'error' })
            limitAdd(apikey);
            let result = await ms.surah(surah)
              res.status(200).json({ status: 200, result: result })
          } catch(err) {
              console.log(err)
              res.status(500).send({ status: 500, message: 'An internal error occurred. Please report via whatsApp wa.me/6285714627920', result: 'error' })
     ...
     async function tafsirsurah(req, res) {
         try {
            ...
            if (limit) return res.status(429).send({ status: 429, message: 'requests limit exceeded (100 req / day), call owner for an upgrade to premium', result: 'error' })
            limitAdd(apikey);
            let result = await ms.tafsirsurah(query)
              res.status(200).json({ status: 200, result: result })
          } catch(err) {
              console.log(err)
              res.status(500).send({ status: 500, message: 'An internal error occurred. Please report via whatsApp wa.me/6285714627920', result: 'error' })
     ...
     async function kisahnabi(req, res) {
         try {
            ...
            if (limit) return res.status(429).send({ status: 429, message: 'requests limit exceeded (100 req / day), call owner for an upgrade to premium', result: 'error' })
            limitAdd(apikey);
            let result = await ms.Searchnabi(nabi)
              res.status(200).json({ status: 200, result: result })
          } catch(err) {
              console.log(err)
              res.status(500).send({ status: 500, message: 'An internal error occurred. Please report via whatsApp wa.me/6285714627920', result: 'error' })
